...
class ReportTemplateStore {
  ...
  private func createTable() {
    ...
    do {
      try database.run(
        reportTemplates.create { table in
  ...
  public func GetReportTemplatesForReport(report_id: Int64) -> [ReportTemplateField] {
    ...
    do {
      for reportTemplate in try database.prepare(self.reportTemplates.filter(reportId == report_id))
      {
        templateList.append(
          ReportTemplateField(
            id: reportTemplate[id], reportId: reportTemplate[reportId], name: reportTemplate[name],
            fieldType: FieldType(rawValue: Int(reportTemplate[fieldType])) ?? FieldType.Text,
