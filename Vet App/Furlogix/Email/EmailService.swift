...
class EmailService {
  ...
  init(
    ...
    reportTemplateRepository: ReportTemplateRepositoryProtocol = DIContainer.shared.resolve(
      type: ReportTemplateRepositoryProtocol.self)!,
  ...
  func gatherReportData(reportId: Int64, presentingController: UIViewController) async {
    ...
    guard
      let fileURL = csvBuilder.buildAndWriteCsv(
        reportName: reportName, entries: entries!, templates: templates)
    else {
    ...
    emailHandler.createAndSendEmail(
      wrapper: emailWrapper, presentingController: presentingController)
