    ...
    NavigationView {
      VStack(spacing: 20) {
        ...
        HStack(spacing: 20) {
          ...
          Button("Submit") {
            let user = User(
              id: -1, name: userViewModel.name, surName: userViewModel.surName,
              email: userViewModel.email)
            let userId = userViewModel.insertUser(user: user)
            if userId != -1 {
              let pet = Pet(
                id: -1, name: userViewModel.petName, type: userViewModel.petType, description: "",
                userId: userId, photoUri: "")
