  ...
  var body: some View {
    VStack {
      ...
      List(reportTemplateViewModel.templatesForReports, id: \.id) { item in
        ReportEntryForm(
          reportName: self.reportViewModel.currentReport?.name ?? "",
          fields: self.reportTemplateViewModel.templatesForReports,
          templateValueMap: self.$templateValueMap,
          timestamp: self.$timestamp)
      }
      Button(action: {
        reportEntryViewModel.insertReportEntry(
          entryValues: self.templateValueMap, reportId: self.reportId, timestamp: self.timestamp)
