...
struct PetsScreenView: View {
  ...
  @State private var headerScale: CGFloat = 0.5
    @State private var toDelete: Int64 = 0
  ...
  var body: some View {
    ZStack {
      ...
      ScrollView {
        LazyVStack(spacing: 20) {
          VStack {
              HeaderSection(title: "My Pets", subtitle: "Manage your beloved companions")
          }
          ...
          VStack(alignment: .leading, spacing: 16) {
            if petViewModel.pets.isEmpty {
              ...
              HStack {
                ...
                HStack {
                  Text(
                    "\(petViewModel.pets.count) \(petViewModel.pets.count == 1 ? "Pet" : "Pets")"
                  )
    ...
    .sheet(isPresented: $showAddDialog){
        AddPetDialog(onAdd: {pet in petViewModel.InsertPet(pet: pet)
            petViewModel.LoadPetsForUser(user_id: userId) })
    }
    .sheet(item: $selectedPet) { pet in
      PetDetailsView(
        ...
        onDelete: { showConfirmDelete = true
            toDelete = selectedPet?.id ?? 0})
    }
    .alert("Delete Pet", isPresented: $showConfirmDelete) {
      Button("Delete", role: .destructive) {
          petViewModel.DeletePet(petId: toDelete)
            petViewModel.LoadPetsForUser(user_id: userId)
          selectedPet = nil
      }
