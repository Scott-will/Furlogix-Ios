          ...
          contentView()
            .navigationDestination(for: AppRoute.self) { route in
              ...
              case .petDashboard(let petId):
                PetDashboardScreenView(
                  petId: petId, onNavigate: { r in routeManager.onNavigate(r) })
              ...
              case .reportEntry(let reportId):
                ReportEntryScreenView(
                  reportId: reportId, onNavigate: { r in routeManager.onNavigate(r) })
              case .manageReports(let petId):
                ManageReportsScreenView(
                  petId: petId, onNavigate: { r in routeManager.onNavigate(r) })
              case .uploadPetPhoto:
