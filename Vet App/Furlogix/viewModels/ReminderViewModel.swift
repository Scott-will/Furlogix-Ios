...
class ReminderViewModel: ObservableObject, ErrorMessageProvider {
  ...
  init(
    remindersRepository: ReminderRepositoryProtocol = DIContainer.shared.resolve(
      type: ReminderRepositoryProtocol.self)!,
    userNotificationService: UserNotificationServiceProtocol = DIContainer.shared.resolve(
      type: UserNotificationServiceProtocol.self)!
  ) {
  ...
  public func deleteReminder(reminder: Reminder) -> Bool {
    ...
    if !result {
      ...
      self.notificationService.cancelReminderNotificationService(
        reminderRequestId: reminder.requestId)
      self.errorMessage = nil
    }
    return result
  }
